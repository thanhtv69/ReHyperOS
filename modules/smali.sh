...
recompile_smali() {
    ...
    if [[ $file_name == *.apk ]]; then
        ...
        rm -rf $base_path/oat/*/$base_name.*
        # dex2oat --dex-file=$file_path --instruction-set=arm64 --compiler-filter=speed --include-patch-information --oat-file=$base_path/oat/arm64/$base_name.odex
